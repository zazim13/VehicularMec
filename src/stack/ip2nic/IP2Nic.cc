...
void IP2Nic::initialize(int stage)
{
    ...
    ofstream myfile;
    csv_filename_gnb_uplayer = fmt::format("logs/ip2nic_gnb_uplayer.csv");
    myfile.open (csv_filename_gnb_uplayer, ios::app);
    std::ifstream file1(csv_filename_gnb_uplayer);
        if ( file1.peek() == std::ifstream::traits_type::eof()) {
      if(myfile.is_open())
      {
          myfile << "timestamp,idFrame,name,module,parent_module,parent_parent_module" << endl;
          myfile.close();
      }
        }

        csv_filename_ue_uplayer = fmt::format("logs/ip2nic_ue_uplayer.csv");
        myfile.open (csv_filename_ue_uplayer, ios::app);
        std::ifstream file2(csv_filename_ue_uplayer);
          ...
          if(myfile.is_open())
          {
            ...
            csv_filename_gnb_sn = fmt::format("logs/ip2nic_gnb_sn.csv");
            myfile.open (csv_filename_gnb_sn, ios::app);
            std::ifstream file3(csv_filename_gnb_sn);
              ...
              if(myfile.is_open())
              {
                ...
                csv_filename_ue_sn = fmt::format("logs/ip2nic_ue_sn.csv");
                myfile.open (csv_filename_ue_sn, ios::app);
                std::ifstream file4(csv_filename_ue_sn);
...
{
    if (!strncmp(msg->getName(), "PingPongPacket-frag-",14)){
        EV<<"rekgr"<<endl;
    }
    if( nodeType_ == ENODEB || nodeType_ == GNODEB)
    {
        ...
        if (msg->getArrivalGate()->isName("upperLayerIn"))
        {
            auto pkt= check_and_cast<Packet *>(msg);
             if (!strncmp(pkt->getName(), "PingPongPacket-frag-",14)){
                    auto chunk = pkt->peekAtFront_<PingPongPacket>(2);
                    auto ch = pkt->getContent();
                    auto pubPk = dynamicPtrCast<const PingPongPacket>(chunk);
                    auto idFrame = pubPk->getIDframe();
                    ofstream myfile;
                    myfile.open (csv_filename_gnb_uplayer, ios::app);
                    if(myfile.is_open())
                         {
                             myfile  << simTime() << "," << idFrame << "," << pkt->getName() << "," << this->getName() <<","<< this->getParentModule()->getName() << "," << this->getParentModule()->getParentModule()->getName() << endl;
                             myfile.close();
                         }
                    //"timestamp,idFrame,name,module,parent_module,parent_parent_module"
                    EV << "zbe"<< endl ;
                    }
        ...
        else if(msg->getArrivalGate()->isName("stackNic$i"))
        {
    		...
    		removeAllSimu5GTags(pkt);
             ...
             if (!strncmp(pkt->getName(), "PingPongPacket-frag-",14)){
                    auto chunk = pkt->peekAtFront_<PingPongPacket>(3);
                    auto ch = pkt->getContent();
                    auto pubPk = dynamicPtrCast<const PingPongPacket>(chunk);
                    auto idFrame = pubPk->getIDframe();
                    EV << "zbe"<< endl ;
