...
void UmRxEntity::enque(cPacket* pktAux)
{
    ...
    auto pktPdu_ = check_and_cast<Packet*>(pktPdu);
    auto pdu_ = pktPdu_->peekAtFront<LteRlcUmDataPdu>();
    auto test = pdu_->getSdu_();
    auto pktSdu_ = check_and_cast<Packet *>(test);


        if (!strncmp(this->getParentModule()->getParentModule()->getParentModule()->getName(), "gNodeB1",5)){
...
void UmRxEntity::initialize()
{
    ...
    ofstream myfile;
    csv_filename_RLC_in = fmt::format("logs/rlc_gnb_input.csv");
    myfile.open (csv_filename_RLC_in, ios::app);
    std::ifstream file1(csv_filename_RLC_in);
        if ( file1.peek() == std::ifstream::traits_type::eof()) {
      if(myfile.is_open())
      {
          myfile << "timestamp,idFrame,framing_info,pduSequenceNumber,SduSizes,NumSdu,data,name,module,parent_module,parent_parent_module,parent_parent_parent_module" << endl;
          myfile.close();
      }
        }

    csv_filename_RLC_out = fmt::format("logs/rlc_gnb_output.csv");
     myfile.open (csv_filename_RLC_out, ios::app);
     std::ifstream file2(csv_filename_RLC_out);
         if ( file2.peek() == std::ifstream::traits_type::eof()) {
       if(myfile.is_open())
       {
           myfile << "timestamp,idFrame,data,name,module,parent_module,parent_parent_module,parent_parent_parent_module" << endl;
           myfile.close();
       }
         }
