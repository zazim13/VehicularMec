...
simsignal_t LtePdcpRrcBase::__handleMessage__Signal =registerSignal("__handleMessage__") ;
...
//    }
    ...
    auto pkt_copy = pkt->dup();


    //pkt_copy->popAtFront<LtePdcpPdu>();

    auto pkt__ = check_and_cast<Packet *>(pkt);

        if (!strncmp(pkt__->getName(), "PingPongPacket-frag-",14)){
            auto chunk = pkt__->peekAtFront<PingPongPacket>();
            auto pubPk = dynamicPtrCast<const PingPongPacket>(chunk);
            auto idFrame = pubPk->getIDframe();
            EV << "zbe"<< endl ;
            //emit(__handleMessage__Signal, pkt__);
...
void LtePdcpRrcBase::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        ofstream myfile;
        csv_filename = fmt::format("logs/pdcp_rrc_{}.csv",timestamp);
        myfile.open (csv_filename, ios::app);
        std::ifstream file(csv_filename);
            if ( file.peek() == std::ifstream::traits_type::eof()) {
          if(myfile.is_open())
          {
              myfile << "timestamp,idFrame,downlink_delay(s),deviceAppAddress_,deviceSimbolicAppAddress_,ueIP,speed,roadID,AppName,ueAppPort,nbRcvMsg" << endl;
              myfile.close();
          }
            }
