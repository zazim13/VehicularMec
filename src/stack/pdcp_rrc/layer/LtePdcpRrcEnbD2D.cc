...
void LtePdcpRrcEnbD2D::handleMessage(cMessage* msg)
{
    ...
    cPacket* pkt__ = check_and_cast<cPacket *>(msg);

    if (!strncmp(pkt__->getName(), "PingPongPacket-frag-",14)){
          auto test = check_and_cast<Packet *>(pkt__);
          auto content = test->getContent();
          auto t = test->peekAtFront<Chunk>();
          try {
             // auto chunk = test->peekAtFront<PingPongPacket>();

              //if (dynamicPtrCast<const PingPongPacket>(chunk )){
